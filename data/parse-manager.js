...
//
// v1.5 修复（相对 v1.4 + 武阳修复版）：
//   1. holdings 单行正则失败 → 改为按位置 7 字段一组解析（兼容每行 1 字段格式）
//   2. holdingPeriods 同样改为 5 字段一组
//   3. managementType 与 assetType 收集错位（管理类型行内容错塞 assetTypes）
//   4. annualReturns tab 分隔：直接 split('\t')，跳过经理名 token
//   5. funds 用 flatMap(l => l.split('\t')) 扁平化（兼容两种格式）
//   6. riskReturn 的 mgrVol/benchVol 误识别（用周期块整段正则）
//   7. riskReturn 的 rank 误识别（用相邻 "收益能力"/"抗风险能力" 锚定）
//   8. 新增中性标签支持（"月度胜率中"），不再硬塞 positive
//   9. 新增标签枚举：股票风格集中/稳定、一拖多、多只FOF持有、近X年波动高、胜率中
//  10. basic.name 字段补全（v1.4 缺）
//  11. basic.bio "显示更多" 截断处理（取最长的曾任/历任段）
//  12. 通用化：函数 extractManager(innerText, id, hint) 纯函数，无 window 依赖
// ============================================================
...
const KNOWN_STYLE = new Set([
  ...
  '换手率较高', '换手率较低', '换手率高',
  '高权益仓位', '低权益仓位',
  '行业集中', '行业分散', '行业较集中', '行业较分散',
  '个股集中', '个股分散',
  '持仓集中度高', '持仓集中度低',
  '权益仓位稳定', '权益仓位波动',
  '股票风格漂移', '股票风格稳定', '股票风格集中'
]);

const KNOWN_SECTOR = new Set([
  '科技', '医药', '消费', '金融', '新能源', '工业', '材料',
  '公用事业', '地产', '可选消费', '必选消费', '通信', '传媒',
  '半导体', '医疗保健', '信息技术'
]);

const PERFORMANCE_POSITIVE = ['收益高', '超额', '胜率高', '捕获比大于1', '胜出', '稳定', '弹性强'];
const PERFORMANCE_NEGATIVE = ['回撤控制能力弱', '回撤控制能力较弱', '波动高', '波动较高', '回撤大', '胜率低'];
...
function extractManager(t, managerId, nameHint = null) {
  ...
  const SECTION_HEADERS = new Set(['投资经验', '持仓风格', '业绩特征', '风险回报']);
  let section = '';
  for (let i = 0; i < lines.length; i++) {
  ...
  if (arSection) {
    ...
    const arSectionTrimmed = arSection.split('管理规模变动')[0] || arSection.split('管理产品列表')[0] || arSection;
    const arLines = arSectionTrimmed.split('\n').filter(l => l.includes('\t'));
    let mgrYears = null;
    let benchYears = null;
    let headerCols = null;
    for (const l of arLines) {
      ...
      const numCount = cols.slice(1).filter(c => !isNaN(parseFloat(c))).length;
      const isNumericRow = numCount >= Math.max(2, Math.floor((cols.length - 1) * 0.7));
      const isMgr = cols.length >= 3 && /[一-龥]/.test(cols[0]) && cols[0] !== '沪深300' && cols[0] !== '总回报%' && isNumericRow;
    ...
    if (mgrYears && benchYears && headerCols && mgrYears.length === headerCols.length && mgrYears.length >= 3) {
      // 修复 iter-008: 不再硬编码 12 列，按头部列数动态生成 yearLabels
      const yearLabels = headerCols.map(h => /今年/.test(h) ? 'ytd' : /任职/.test(h) ? 'sinceInception' : parseInt(h, 10));
      annualReturns = {
        benchmark: '沪深300',
        dataAsOf: t.match(/\*数据截止日期:\s*(\d{4}年\d{2}月\d{2}日)/)?.[1] || null,
        unit: '%',
        returns: []
      };
      for (let i = 0; i < yearLabels.length; i++) {
        ...
        else if (yearLabels[i] === 'sinceInception') annualReturns.sinceInception = item;
  ...
  const industryMatch = t.match(/持仓占比%\s+([\s\S]+?)(?=股票风格箱|前十大持仓)/);
  if (industryMatch) {
    const indLines = industryMatch[1].trim().split('\n').filter(l => l.trim());
    const items = [];
  ...
  // ---------- 6. 股票风格箱 ----------
  let styleBox = null;
  const sbMatch = t.match(/股票风格箱\s+(20\d{2}-\d{2}-\d{2})\s+价值\s+平衡\s+成长\s+大盘\s+([\d\s]+)中盘\s+([\d\s]+)小盘\s+([\d\s]+)/);
  if (sbMatch) {
    ...
    styleBox = {
      asOf: sbMatch[1],
      cells: {
        '大盘价值': bigNums[0], '大盘平衡': bigNums[1], '大盘成长': bigNums[2],
        '中盘价值': midNums[0], '中盘平衡': midNums[1], '中盘成长': midNums[2],
        '小盘价值': smallNums[0], '小盘平衡': smallNums[1], '小盘成长': smallNums[2]
      }
    };
    ...
    styleBox.styleBias = styleMax === 'growth' ? '成长' : styleMax === 'value' ? '价值' : '平衡';
  }
  ...
  if (fundsSection) {
    ...
    const tokens = fundsSection.split('\n')
      .flatMap(l => l.split('\t'))
      .map(s => s.trim())
      .filter(s => s);
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (codeMatch) {
        ...
        for (let j = i - 1; j >= Math.max(0, i - 4); j--) {
          const v = tokens[j];
          if (v === '代表产品') { isRep = true; continue; }
          if (v.length > 4 && !/^\d+$/.test(v) && !['现管基金', '历史任职', '名称', '基金代码', '基金规模(亿元)', '晨星分类', '任职日期', '在任时长', '任职回报', '同期同类回报'].includes(v)) {
            name = v;
            break;
          }
        }
        const scale = tokens[i + 1] && tokens[i + 1] !== '-' ? tokens[i + 1] : null;
        ...
        let catL1 = null;
        if (category && category.includes(' - ')) catL1 = category.split(' - ')[0];
        ...
        funds.push({
          ...
          excessReturn: !isNaN(tenureRet) && !isNaN(benchRet) ? tenureRet - benchRet : null,
  ...
  if (holdingsMatch) {
    ...
    const hLines = holdingsMatch[2].split('\n').map(l => l.trim()).filter(l => l);
    const items = [];
    for (let i = 0; i + 6 < hLines.length; i += 7) {
      ...
      if (codeLooksValid && !isNaN(w)) {
        items.push({
          ...
          shareChange: hLines[i + 5] !== '—' && hLines[i + 5] !== undefined
  ...
  if (periodsMatch) {
    ...
    // 修复 #2: 按 5 字段一组解析
    const pLines = periodsMatch[2].split('\n').map(l => l.trim()).filter(l => l);
    const items = [];
    for (let i = 0; i + 4 < pLines.length; i += 5) {
      ...
      if (!isNaN(q)) {
        items.push({
          name: pLines[i],
          quarters: q,
          mktValue: pLines[i + 2] !== '—' && pLines[i + 2] !== undefined
            ? parseFloat(pLines[i + 2])
            : null,
          currentRank: pLines[i + 3] !== '—' && pLines[i + 3] !== undefined
            ? pLines[i + 3]
            : null,
          sector: pLines[i + 4]
        });
  ...
  const education = (function() {
    // 顺序：先试行首短字段（独立段），再在整段文本中搜
    // iter-010 扩展：兼容"微电子学与固体电子学硕士"等"X学与Y学硕士"组合
    // 注意顺序：先匹配长串（"博士研究生"/"工商管理硕士"），再单独"博士"/"硕士"
    const eduRegex = /(博士研究生|硕士研究生|博士研究生学历|硕士研究生学历|管理学博士|经济学博士|工学博士|理学博士|金融学博士|会计学博士|管理学硕士|经济学硕士|工学硕士|理学硕士|金融学硕士|会计学硕士|法学硕士|文学硕士|工商管理硕士|MBA|EMBA|[一-龥]+学(?:与[一-龥]+学)?硕士|[一-龥]+学(?:与[一-龥]+学)?博士|博士|硕士|本科[科学历]?)/;
    for (const l of lines) {
...
function asciiName(name) {
  // 简单映射：经理中文姓名 → 拼音文件名（仅作文件名，不影响内容）
  // 如果是中文（不在 ASCII 范围），用拼音映射或退回 'manager'
  const map = {
    '刘元海': 'liuyuanhai',
    '武阳': 'wuyang',
    '郑希': 'zhengxi',
...
function printStats(data) {
  ...
  console.log(`basic.managementType: ${JSON.stringify(data.basic.managementType)}`);
  ...
  console.log(`labels.sectorPreference: ${data.labels.sectorPreference.length} 项`);
  console.log(`labels.performance: ${data.labels.performance.length} 项 (正${data.labels.performance.filter(p => p.polarity === true).length}/负${data.labels.performance.filter(p => p.polarity === false).length}/中${data.labels.performance.filter(p => p.polarity === null).length})`);
  console.log(`riskReturn.current.period: ${data.riskReturn.current?.period}`);
  console.log(`riskReturn.current.managerReturn: ${data.riskReturn.current?.managerReturn}%`);
  console.log(`riskReturn.current.managerVol: ${data.riskReturn.current?.managerVol}%`);
  console.log(`annualReturns.returns: ${data.annualReturns?.returns.length || 0} 年`);
  console.log(`industryAllocation.current: ${data.industryAllocation?.current.length || 0} 行业`);
  console.log(`styleBox: ${data.styleBox ? Object.values(data.styleBox.cells).filter(v => typeof v === 'number').length + ' cells, sizeBias=' + data.styleBox.sizeBias + ', styleBias=' + data.styleBox.styleBias : 'null'}`);
  console.log(`funds: ${data.funds.length} 只`);
  console.log(`topHoldings.quarterly: ${data.topHoldings.quarterly?.holdings.length || 0} 持仓`);
  console.log(`holdingPeriods.quarterly: ${data.holdingPeriods.quarterly?.items.length || 0} 持有期`);
}
...
function validate(data) {
  ...
  const checks = [
    ...
    ['annualReturns.returns >= 5', (data.annualReturns?.returns.length || 0) >= 5],
    ['annualReturns.sinceInception', !!data.annualReturns?.sinceInception],
    ['industryAllocation.current >= 1', (data.industryAllocation?.current.length || 0) >= 1],
    ['styleBox 9 cells', data.styleBox && Object.values(data.styleBox.cells).filter(v => typeof v === 'number').length === 9],
    ['funds >= 1', data.funds.length >= 1],
    ['topHoldings.quarterly >= 5', (data.topHoldings.quarterly?.holdings.length || 0) >= 5],
    ['holdingPeriods.quarterly >= 3', (data.holdingPeriods.quarterly?.items.length || 0) >= 3]
  ];
  let pass = 0;
...
function main() {
  ...
  if (!/^\d{4,7}$/.test(String(managerId))) {
    console.error(`✗ managerId 必须是 4-7 位数字（晨星内部 ID），收到：${managerId}`);
  ...
  if (rawHint === '返回') {
    console.error('✗ nameHint 是 "返回"，这是页面按钮不是姓名，请传正确的经理名');
    process.exit(1);
  }
  ...
  const outDir = path.join(__dirname, 'raw', 'morningstar');
...
module.exports = { extractManager, KNOWN_EXPERIENCE, KNOWN_STYLE, KNOWN_SECTOR };