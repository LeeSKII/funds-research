...
>
> **Date of probe sweep:** 2026-06-21. **Engine production filter:** `engine/core/config/universe.json` (`search_filter` key → 506 funds = the validation BASELINE).
...
- `token` is an RS256 JWT harvested from a logged-in browser's `localStorage['token']` (~14-day expiry, see `engine/secrets/token.json`).
...
There is **no `/config` or `/criteria` endpoint**. The field schema is a single ~29KB object-literal array embedded in the minified `screener.js` bundle (byte offset 38418–67790, anchored at `let t=[{label:\`基本信息\`...`). The 3 `/cn-api/v2/template/*` paths are user-saved-filter CRUD only (GET `config-list` returns saved blobs, not a schema). The bundle IS the schema; the engine keeps a parsed copy at `engine/tmp/filter-block.json`.
...
Source: `engine/core/config/universe.json` → `search_filter`. The body sent to `search/es?source=local` each day. Empirical result: **506 funds** (well under the 1000-row cap; ~500 rows of headroom).
