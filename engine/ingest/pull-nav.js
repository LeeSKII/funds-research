// engine/ingest/pull-nav.js — standalone NAV puller (the Node half of the browser-hybrid architecture).
//
...
//
// USAGE:
//   node engine/ingest/pull-nav.js <6-digit-id> [catAvgSecId] [-o <out.json>] [--start DATE] [--end DATE]
//   node engine/ingest/pull-nav.js 006502
//
 ...
 * @param {object} [opts]
 * @param {string} [opts.token]  JWT (default: read from engine/secrets/token.json).
...
function main(argv) {
  ...
  if (!id) { console.error('Usage: node engine/ingest/pull-nav.js <6-digit-id> [catAvgSecId] [-o <out>] [--start DATE] [--end DATE]'); process.exit(1); }
