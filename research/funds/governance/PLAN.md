...
- [ ] **parse-fund.js (detail scraper) — IMMEDIATE NEXT.** Navigate each row's `detailUrl` (`/fund/<id>.html`) → parse rendered DOM (Brinson 归因 / risk / downside-capture / calendar returns / top-10 holdings / manager / fees) + `growth-data` NAV pull (needs secId). Spec: `research/funds/docs/fund-detail-api.md`. Outputs a per-fund dossier JSON. This is the core of Plan 2.
