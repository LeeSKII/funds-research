// research/funds/analyze/parse-fund.js — v2.0.0 ORCHESTRATOR (page-structure-aligned).
//
// The dossier top-level now mirrors the morningstar.cn /fund/<id>.html page TABS. Each tab is its
// own extractor in research/funds/analyze/sections/<name>.js (built/tested in isolation), and THIS file is
...
function main(argv) {
  ...
  if (!args[0]) {
    console.error('Usage: node research/funds/analyze/parse-fund.js <snapshot.json> [code] [-o <out.json>]');
    process.exit(1);
  }
