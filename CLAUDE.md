...
- **路径**：`research/raw-snapshots/<source>-<id>-<YYYYMMDD>.json`
...
node research/scripts/parse-manager.js \
  research/raw-snapshots/morningstar-<id>-<date>.json \
  <id> <name>
# 例：node research/scripts/parse-manager.js research/raw-snapshots/morningstar-191993-20260619.json 191993 狄星华
```

- **输入**：第 1 步的 `raw-snapshots/*.json`
- **输出**：`data/raw/morningstar/manager-<id>-<name>.json`（符合 `data/manager-schema.json`）
...
```bash
node research/scripts/validate-manager.js                              # 全量
node research/scripts/validate-manager.js data/raw/morningstar/<file>  # 单个
```
...
│   └── raw/morningstar/             # 第 2 步产物：结构化 manager JSON
├── research/
│   ├── raw-snapshots/               # 第 1 步产物：innerText 原始快照
│   └── scripts/                     # 第 2-3 步：parse-manager.js + validate-manager.js
├── reports/                         # 研究报告（output 改名为 reports）
