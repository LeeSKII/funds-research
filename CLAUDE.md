...
| **基金研究** | `engine/` | [`GUIDE.md`](./engine/GUIDE.md) | ① 筛选条件 → 基金列表　② 列表 → 基金详情 dossier → 研究 |
...
- **安全**：`engine/secrets/` 含 live JWT（gitignored），永不提交、永不回显；原始网络捕获含 live token。
...
├── engine/          → GUIDE.md        # 基金研究（筛选 → 详情）
├── research/managers/ → GUIDE.md      # 基金经理研究（4 步）
