...
- **输出**：`data/raw/manager/manager-<id>-<name>.json`（符合 `data/manager-schema.json`）
...
node research/managers/scripts/validate-manager.js data/raw/manager/<file>  # 单个
```
...
parse-manager 自动写到 `data/raw/manager/manager-<id>-<name>.json`。**用户没指定路径就用默认位置**；用户指定则用 `node research/managers/scripts/parse-manager.js ... -o <path>`（未来扩展）。
...
- `research/funds/`：基金分析子模块，**同样遵循 4 步结构**
  ...
  - `data/raw/funds/fund-<code>.json`（符合 `data/fund-schema.json`，未来建）
...
│   └── raw/manager/                # 第 2 步产物：结构化 manager JSON
├── research/
